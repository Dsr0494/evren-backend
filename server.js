...
const cotizacionSchema = new mongoose.Schema({ id: { type: String, required: true, unique: true } }, { strict: false, timestamps: true });
...
const perfilSchema = new mongoose.Schema({ usuario: { type: String, required: true, unique: true }, avatar: { type: String } });
const Perfil = mongoose.models.Perfil || mongoose.model('Perfil', perfilSchema);
const equipoSchema = new mongoose.Schema({ imei: { type: String, required: true, unique: true }, modelo: { type: String, required: true }, marca: { type: String }, sku: { type: String }, iccid: { type: String }, proveedor: { type: String }, referencia: { type: String }, folioCompra: { type: String }, sucursal: { type: String, default: 'GENERAL' }, estado: { type: String, default: 'Disponible' }, fechaIngreso: { type: String }, fechaRegistro: { type: Date, default: Date.now } });
const Equipo = mongoose.models.Equipo || mongoose.model('Equipo', equipoSchema);
const simSchema = new mongoose.Schema({ icc: { type: String, required: true, unique: true }, tipo: { type: String, default: 'SIM' }, modelo: { type: String }, sku: { type: String }, proveedor: { type: String }, referencia: { type: String }, folioCompra: { type: String }, sucursal: { type: String, default: 'GENERAL' }, estado: { type: String, default: 'Disponible' }, fechaIngreso: { type: String }, fechaRegistro: { type: Date, default: Date.now } });
const Sim = mongoose.models.Sim || mongoose.model('Sim', simSchema);
const usuarioSchema = new mongoose.Schema({ nombre: { type: String, required: true, uppercase: true }, usuario: { type: String, required: true, unique: true, uppercase: true }, contrasenaEncriptada: { type: String, required: true }, categoria: { type: String, required: true, uppercase: true }, organizacion: { type: String, required: true, uppercase: true }, sucursal: { type: String, required: true, uppercase: true }, nivelAcceso: { type: String, required: true, uppercase: true, enum: ['ADMINISTRADOR', 'USUARIO'] }, dosPasosActivo: { type: Boolean, default: false }, dosPasosSecreto: { type: String, default: null }, activo: { type: Boolean, default: true }, fechaCreacion: { type: Date, default: Date.now } });
...
const uploadBase64ToS3 = async (base64String, folder, fileName) => {
  ...
  try {
    ...
    const type = matches[1]; const buffer = Buffer.from(matches[2], 'base64');
    const uniqueName = `${folder}/${Date.now()}_${Math.random().toString(36).substring(7)}_${fileName.replace(/[^a-zA-Z0-9.]/g, '_')}`;
    const command = new PutObjectCommand({ Bucket: BUCKET_NAME, Key: uniqueName, Body: buffer, ContentType: type });
    await s3Client.send(command);
    return `https://${BUCKET_NAME}.s3.${process.env.AWS_REGION}.amazonaws.com/${uniqueName}`;
  } catch (error) { return base64String; }
};
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (user.nivelAcceso !== "ADMINISTRADOR" && user.sucursal !== sucursal) {
      ...
      if (!esMesaControl) return res.status(403).json({ mensaje: `Acceso denegado al módulo ${sucursal}` });
    }
...
app.post('/api/auth/2fa/disable', async (req, res) => {
  try {
    ...
    user.dosPasosActivo = false; user.dosPasosSecreto = null; await user.save();
    res.json({ mensaje: "Seguridad 2FA desactivada." });
  } catch (error) { res.status(500).json({ mensaje: "Error del servidor." }); }
});

app.get('/api/usuarios', async (req, res) => {
  try { const usuarios = await Usuario.find({ activo: true }).select('-contrasenaEncriptada -dosPasosSecreto').sort({ fechaCreacion: -1 }); res.json(usuarios); } catch (error) { res.status(500).json({ mensaje: "Error al obtener usuarios." }); }
});
...
app.post('/api/usuarios', async (req, res) => {
  try {
    ...
    await nuevoUsuario.save(); const usuarioSafe = nuevoUsuario.toObject(); delete usuarioSafe.contrasenaEncriptada; delete usuarioSafe.dosPasosSecreto;
...
app.post('/api/perfil/password', async (req, res) => {
  try {
    ...
    user.contrasenaEncriptada = bcrypt.hashSync(passNueva, 10); await user.save();
...
app.post('/api/perfil/avatar', async (req, res) => {
  try {
    ...
    if (avatar && avatar.includes('base64,')) avatarUrl = await uploadBase64ToS3(avatar, 'avatares', `${usuario}_avatar.jpg`);
...
app.get('/api/cotizaciones', async (req, res) => { 
  try { const cotizacionesLigeras = await Cotizacion.find().sort({ _id: -1 }); res.json(cotizacionesLigeras); } 
...
// 🚀 CARRILES PARALELOS A AWS PARA EL VENDEDOR (SÚPER RÁPIDO)
...
// 🚀 CARRILES PARALELOS A AWS PARA MESA Y VENDEDOR (ACTUALIZACIONES)
app.put('/api/cotizaciones/:id', async (req, res) => {
  try {
      ...
      const cotizacionActualizada = await Cotizacion.findOneAndUpdate( { $or: condicionesBusqueda }, { $set: bodyData }, { new: true } );
...
app.post('/api/inventario/cargar-csv', upload.single('archivoCsv'), (req, res) => {
  ...
  const filasDelCsv = []; let numeroFila = 1;
...
app.post('/api/compras/ingreso-masivo', async (req, res) => {
  try {
    ...
    await mongoose.connection.collection('historialCompras').insertOne({ 
        ...
        articulosInsertados: insertados, usuarioCarga: usuarioCarga || 'DESCONOCIDO', timestamp: new Date() 
    });
...
app.use((err, req, res, next) => {
  if (err.type === 'entity.too.large') {
    return res.status(413).json({ error: "El archivo enviado es demasiado pesado.", mensaje: "Sube archivos más ligeros." });
  }
  next(err);
});
...
const sembrarUsuariosIniciales = async () => {
  try {
      ...
      for (let u of usuariosIniciales) {
         ...
         if (!existe) { u.contrasenaEncriptada = bcrypt.hashSync(String(u.contrasena), 10); await Usuario.create(u); }
      }
  } catch (error) {}
};
